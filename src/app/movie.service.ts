...
export class MovieService {
  ...
  private mapMoviePathImages<T extends { poster_path: string }>(
    movies: T[]
  ): T[] {
    ...
    return movies.map((movie) => ({
      ...movie,
      poster_path: this.hasPosterPath(movie)
        ? images_url + movie.poster_path
        : this.dummy_image,
    }));
  }

  private mapMovieBackdrop<T extends { backdrop_path: string }>(
    movies: T[]
  ): T[] {
    const images_url = environment.images_url;
    ...
    return movies.map((movie) => ({
      ...movie,
      backdrop_path: this.hasBackdrop(movie)
        ? images_url + movie.backdrop_path
        : this.dummy_image,
    }));
  }
  ...
  getMovies(pageNumber?: number) {
    ...
    return this.httpClient
      .get<IPaginationResult<IMovieResult>>(moviesUrl, { params: params })
      .pipe(
        map((movie) => ({
          movies: this.mapMoviePathImages(movie.results),
          page: movie.page,
  ...
  getMovie(id: number) {
    ...
    let movieDetails = this.httpClient
      .get<IMovieDetails>(movieUrl, { params: params })
      .pipe(
        map(
          (movie) =>
            ({
              ...movie,
              poster_path: environment.images_url + movie.poster_path,
              backdrop_path: environment.images_url + movie.backdrop_path,
            } as IMovieDetails)
        )
      );
  ...
  getRecommendation(id: number) {
    ...
    return this.httpClient
      .get<IPaginationResult<IMovieRecommendation>>(movieUrl, {
        params: params,
      })
      .pipe(
        map(
  ...
  searchMovies(query: string, pageNumber?: number): Observable<IMovieItem[]> {
    ...
    return this.httpClient
      .get<IPaginationResult<IMovieResult>>(searchUrl, { params: params })
      .pipe(
        map((response) => response.results),
        map((movies) =>
          movies.map((item) => ({ id: item.id, name: item.title }))
        )
      );
  }
  ...
  rateMovie(id: number, rating: number) {
    ...
    const session_id =
      this.storageService.getFromStroage<string>('guest_token') || '';

    let httpParams = new HttpParams();

    const params = httpParams
      .set('api_key', environment.api_key)
      .set('guest_session_id', session_id);
    ...
    return this.httpClient.post<IResponse>(ratingUrl, payload, {
      params: params,
      headers: headers,
    });
  }
